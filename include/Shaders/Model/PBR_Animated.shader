...
void main(void)
{
	...
	else
	{
		...
		vec3 ambient_diffuse_out = albedo.xyz;	
		...
		float roughnessmip 		= 7 - (pow(fgloss, 3) * 7);	
		...
		ambient_diffuse_out *= srgb_to_lin(textureLod(texture7, worldnormal, 11), gamma).xyz;	
		ambient_diffuse_out *= invmetallic;
						
		ambient_specular_out = srgb_to_lin(textureLod(texture7, cubecoord, roughnessmip), gamma).xyz;	
		float exponent = pow((1.0f - dot(normal, reflectvec)), 5.0f);		
