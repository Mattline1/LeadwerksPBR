...
void main(void)
{
	...
	vec3 N = texture(texture8,ex_texcoords0).xzy;
	...
	fragData0 = srgb_to_lin(outcolor, 2.2);
	int materialflags=1+16;//16 for decal mode
	
	fragData1 = vec4(normal * 0.5 + 0.5, materialflags / 255.0 );
	float fakeRoughness = mix(0.05, outcolor.r+outcolor.g+outcolor.b*0.333333, 0.2);
	fragData2 = vec4(fakeRoughness, 0.0, 0.04, 0.0);
}
